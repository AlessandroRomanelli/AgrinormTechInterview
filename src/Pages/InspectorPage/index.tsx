...
import {Batch, JSONBatch} from "../../Types/Batch";
...
const InspectorPage: React.FC = () => {
	...
	return <div className={'inspectors'}>
		...
		<p>Here you will be able to view all tasks that are pending and have been assigned to you. You will find detailed information about each task including the product details, the inspection criteria, and the deadline for completion. You will also have the ability to update the status of the task as you progress through the inspection process. This page serves as your central hub for managing your assigned tasks and ensuring that they are completed on time and to the highest standards. If you have any questions or need assistance, please do not hesitate to reach out to our support team.</p>
		{ !inspectorId && <>
		...
		{ inspectorId && !tasks.length && <p className={'text-muted'}>It seems like you have no tasks registered under your ID. Please, get in touch with your manager to be assigned to a task.</p>}
		{tasks.length > 0 && <TasksVisualizer tasks={sort(tasks, sortBy, sortDesc)} setTasks={setTasks} sortBy={sortBy} sortDesc={sortDesc}
